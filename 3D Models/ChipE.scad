...
// Part to print
part="full"; // [full:Preview Bot,left_foot:Left Foot,right_foot:Right Foot,base:Base section,middle:Middle section,top:Top Plate,head:Head,arm:Arm,bracket2:Printable Bracket (leg)]
global_width=100; // [70:1:200]
global_depth=70; // [70:1:200]
// Global rounding radius
global_radius=0; // [0:.1:50]
print_standoffs=0; // [0:No,1:Yes]
// Use RobotGeek parts
rgstock=1; // [0:No,1:Yes]
...
module bracket(thick=bracket_thick,servo=!rgstock) {
  ...
  for(m=[0,1]) mirror([m,0]) {
    difference(){
      ...
      translate([-23.6-thick,0,bracket_length-30+thick]) rotate([0,90]) {
        if(!servo) hornholes(thick,1);
        else servoholes(thick);
        if(rgstock)
       for(y=[0:-8.25:bracket_length*-1+53])
          render()
    ...
    if(!servo) for(m=[0,1]) mirror([0,m])
      translate([-23.75,10.2,2.5]) rotate([0,90,0]) intersection() {
...
module hornholes(thick,square=0) {
  ...
  if(square)
    translate([-5,-5]) cube([5,10,thick+2]);
}

module foot(bot_offset=0) {
  ...
  translate(bot_offset?[121.11,38]:[]) {
    translate([-26,17,5.11]) rotate([90,0]) {
      if(part=="full")
        translate([0,foot_thick-5]) color("black") servo();
      //else %servo();
    }
...
//  base_heigt=37;
  ...
  *%mirror_xoffset(0) translate([-10-leg_offset/2,-13]) rotate([0,0,90]) servo();
  difference() {
    color(part=="full"?"green":false) union() {
    ...
    for(m=[0,1]) mirror([m,0]) {
        translate([-35.8-leg_offset/2,-20]) cube([29.6,55,7]);
        if(print_standoffs)
    ...
    mirror_xoffset(0)
    translate([-30-leg_offset/2,-13.4,-.01]) {
      cube([20,40.8,7.02]);
  ...
  difference() {
    union() {
      difference() {
        ...
        translate([(leg_offset-10)/2,0]) translate([15,-13,-.01]) cube([20,20,3]);
        translate([(leg_offset-10)/-2,0]) {
...
{
  rotate([0,0,45]) translate([(global_radius)/2,0]) rotate([0,0,-45])
  children();
}
...
module bot() {
  translate([77.58,0]) {
    for(m=[0,1]) mirror([m,0]) translate([-82.58+leg_offset/2,-36.94]) {
      foot(1);
  ...
  base_bottom=bracket_length+(foot_thick-5);
  ...
  translate([-20-leg_offset/2,-3,base_bottom]) rotate([180,0,90]) {
    if(rgstock) {
      legs();
    } else {
      color("black")
...
module legs() {
    bracket();
    translate([0,40+leg_offset,0]) bracket();
}
